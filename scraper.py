...
# Use LV or EN page — both are supported; LV by default
...
# Hard safety cap in case pagination loops go wild (per combo)
MAX_PAGES_HARD_CAP = int(os.getenv("MAX_PAGES_HARD_CAP", "5000"))

def row_id(r: dict) -> str:
    # Prefer BIS case number if present
    if r.get("bis_number"):
...
def parse_table(html: str) -> list[dict]:
    ...
    for tr in table.find_all("tr"):
        ...
        # Ignore if moved to Construction works (not actual anymore)
...
async def click_next(page) -> bool:
    # Try common "Next" buttons/links in LV/EN
    for selector in [
...
async def apply_filters(page, authority, phase, ctype):
...
async def scrape_combo(page, authority, phase, ctype):
    await apply_filters(page, authority, phase, ctype)

    results = []
    pages = 0
    while pages < MAX_PAGES_HARD_CAP:
        html = await page.content()
        rows = parse_table(html)
        if not rows:
            break
        # Tag combo on each row
        for r in rows:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for authority in ALL_AUTHORITIES:
            for phase in PHASES:
                for ctype in TYPES:
                    chunk, walked = await scrape_combo(page, authority, phase, ctype)
                    all_rows.extend(chunk)
                    total_pages_walked += walked

        await browser.close()

    # Build maps for diff
    os.makedirs("reports", exist_ok=True)
    prev_path = "reports/latest.csv"
    prev = pd.read_csv(prev_path, dtype=str).fillna("").to_dict("records") if os.path.exists(prev_path) else []
    ...
    for i in set(prev_map).intersection(cur_map):
        ...
        fields = [k for k in ["phase","construction_type","address","object"]
                  if (a.get(k,"") != b.get(k,""))]
        if fields:
    ...
    lines = [
        ...
        f"- Pārlapoto lapu skaits: {total_pages_walked}",
        "",
        "## Jaunie"
    ]
    for i in new_ids:
        r = cur_map[i]
        lines += [f"- **{r.get('authority','?')}** — {r.get('bis_number','?')} — {r.get('address','?')} — {r.get('object','?')} — {r.get('phase','?')} — {r.get('construction_type','?')}  " +
    ...
    # Print a small JSON summary to logs
    print(json.dumps({
        "total_rows": len(cur_map),
        "new": len(new_ids),
        "removed": len(gone_ids),
        "updated": len(changed),
        "pages_walked": total_pages_walked
    }, ensure_ascii=False))
